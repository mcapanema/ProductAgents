...
| `_sidecar_main.py` | PyInstaller entry point for the frozen IPC sidecar binary the packaged Tauri app spawns instead of `uv run productagents ipc`. No argv parsing — `run()` just calls `cli.main(["ipc"])`, so workspace resolution/activation/logging stay identical to the dev path. |
